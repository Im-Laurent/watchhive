...
- 진동수: 시간당 ${input.bph}회
- 일오차: ${input.rateSecondsPerDay.toFixed(1)}초/일
- 비트에러: ${input.beatErrorMs.toFixed(2)}ms

참고로 정비된 시계 기준으로는 대략 일오차 ±10초/일 이내, 비트에러 0.5ms 이내면 우수한 편으로 봅니다.
이 수치를 바탕으로 현재 상태를 한국어 2~4문장으로 설명해주세요. 다음 규칙을 반드시 지키세요:
- "안녕하세요" 같은 인사말이나 서두 없이, 첫 문장부터 곧바로 측정 결과 설명으로 시작하세요.
- 지금 수치가 어떤 상태인지 설명만 하세요. 사용자가 무엇을 해야 하는지는 쓰지 마세요.
  "다시 측정해 보세요", "전문가에게 맡겨보세요", "조정을 받아보시길 권합니다", "~하는 것을
  추천합니다" 같은 권유·제안·행동 유도 문장은 어떤 형태로도 넣지 마세요.
- 확정적인 고장 진단이나 특정 부품 교체를 언급하지 마세요.
- 스마트폰 마이크 측정의 한계나 "참고용으로만 봐달라"는 단서는 넣지 마세요. 화면 아래에 별도 안내가 이미 있어 중복됩니다.
- 전문 용어는 최소화하고 일반인이 이해하기 쉽게 설명하세요.
- 과도하게 걱정을 주거나 반대로 과도하게 안심시키지 마세요.
- 용어는 '진동수', '일오차', '비트에러'로만 쓰세요. BPH, Rate, Beat Error 같은 영어 표기는 쓰지 마세요.`;
}
